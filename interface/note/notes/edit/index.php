...
if (isset($_GET['popupEditor'])) {
	require ROOT . '/interface/common/note/headerForPopupEditor.php';
} else {
						...
						<script type="text/javascript">
							//<![CDATA[
								...
								function setEnclosure(value) {
									...
									var request = new HTTPRequest("POST", "<?php echo $context->getProperty('uri.blog');?>/note/notes/attach/enclosure/");
...
?>
									...
									self.loadTemplate = function (templateId,title) {
										...
										var request = new HTTPRequest("POST", "<?php echo $context->getProperty('uri.blog');?>/note/notes/loadTemplate/");
									...
									self.saveEntry = function () {
										...
										if(self.isSaved == true) {
											var request = new HTTPRequest("POST", "<?php echo $context->getProperty('uri.blog');?>/note/notes/draft/"+entryManager.entryId);
										} else {
											var request = new HTTPRequest("POST", "<?php echo $context->getProperty('uri.blog');?>/note/notes/add/");
										}
										if(self.autoSave != true) {
									...
									this.openPreviewPopup = function () {
										window.open("<?php echo $context->getProperty('uri.blog');?>/note/notes/preview/"+self.entryId, "previewEntry"+self.entryId, "location=0,menubar=0,resizable=1,scrollbars=1,status=0,toolbar=0");
									}
									self.saveAndReturn = function () {
										...
										if (data == null)
											return false;
										if(self.isSaved == true) {
											var request = new HTTPRequest("POST", "<?php echo $context->getProperty('uri.blog');?>/note/notes/finish/"+entryManager.entryId);
										} else {
											var request = new HTTPRequest("POST", "<?php echo $context->getProperty('uri.blog');?>/note/notes/finish/");
										}
...
<?php
$param = array(
		'uploadPath'=>       $context->getProperty('uri.blog')."/note/notes/attachmulti/",
		'singleUploadPath'=> $context->getProperty('uri.blog')."/note/notes/attach/",
		'deletePath'=>       $context->getProperty('uri.blog')."/note/notes/detach/multi/",
		'labelingPath'=>     $context->getProperty('uri.blog')."/note/notes/attachmulti/list/",
		'refreshPath'=>      $context->getProperty('uri.blog')."/note/notes/attachmulti/refresh/",
		'fileSizePath'=>     $context->getProperty('uri.blog')."/note/notes/size?parent=");
...
<?php
if (isset($_GET['popupEditor']))
	require ROOT . '/interface/common/note/footerForPopupEditor.php';
else
